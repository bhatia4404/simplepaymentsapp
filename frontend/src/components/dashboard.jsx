...
import { usersFilterAtom } from "../store/atoms/users";
import { currentUserData } from "../store/atoms/currentUser";
import { Suspense } from "react";

export function Dashboard() {
  const setFilter = useSetRecoilState(usersFilterAtom);
  const curUser = useRecoilValue(currentUserData);

  return (
    <div className="bg-white p-3 flex flex-col gap-3">
      <Appbar firstName={curUser.user.firstName}></Appbar>
      <hr className="mt-3 mb-3" />

      <p className="font-bold ml-4">Users</p>
      <input
        type="text"
        className="outline-0 w-[70%] border rounded border-gray-300 pl-2 ml-4"
      ...
      <Balance balance={curUser.balance} />
      <Suspense
        fallback={
